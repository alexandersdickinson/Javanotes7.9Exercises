...
public class SimpleParser4 {
    ...
    private static class BinOpNode extends ExpNode{
		...
		ExpNode derivative(){
            ...
            ExpNode y = right;
            switch (op) {
            case '+':  return new BinOpNode('+', x, y);
            case '-':  return new BinOpNode('-', x, y);
            case '*':  return new BinOpNode('+', new BinOpNode('*', x, y.derivative()), new BinOpNode('*', x.derivative(), y));
            default:  
				return new BinOpNode('/', new BinOpNode('-', new BinOpNode('*', y, x.derivative()), new BinOpNode('*', x, y.derivative())), new BinOpNode('*', y, y));
            }
		}
        void  printStackCommands() {
        ...
        while (true) {
            ...
            try {
				...
				for(double i = 1; i < 4; i++){
					...
					System.out.println("Value of the derivative is " + exp.derivative().value(i));
